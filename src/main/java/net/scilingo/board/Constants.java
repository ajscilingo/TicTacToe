/**
 * 
 */
package net.scilingo.board;

/**
 * @author ajscilingo
 *
 */
public class Constants {

	public static final String NEWLINE = System.getProperty("line.separator");
	public static final String HTML_LINE_BREAK = "<BR>";
	public static final char X_SYMBOL = 'X';
	public static final char O_SYMBOL = 'O';
	public static final char UNOCCUPIED_SPACE = '_';
	public static final char CELL_SPACER = '|';
	public static final int ONE_PLAYER = 1;
	public static final int TWO_PLAYER = 2;
	
	public static final String ONE_OR_TWO_PLAYERS_MSG = "Choose 1 or 2 Players!";
	public static final String INVALID_PLAYER_SELECTION_MSG = "Invalid Player Selection";
	public static final String PLAY_AGAIN_MSG = "Start a new game? Y/N";
	
	public static final String YES = "YES";
	public static final String Y = "Y";
	public static final String NO = "NO";
	public static final String N = "N";

	// Rows and Columns for a Tic-Tac-Toe Board
	public static final int NUM_OF_COLS = 3;
	public static final int NUM_OF_ROWS = 3;

}